...
angular.module('google.places', [])
   ...
   */
  .value('googleMaxChecks', 10)
  .factory('googleApiPromise', ['$window', '$interval', '$q', 'googleMaxChecks', function ($window, $interval, $q, googleMaxChecks) {
    var deferred = $q.defer();
    var checkCount = 0;

    var stop = $interval(function(){

      if (checkCount >= googleMaxChecks && !$window.google){
        deferred.reject('Global `google` var missing. Did you forget to include the places API script?');
  ...
  .directive('gPlacesAutocomplete',
        [ '$parse', '$compile', '$timeout', '$document', 'googleApiPromise',
        function ($parse, $compile, $timeout, $document, googleApiPromise) {
            ...
            return {
                ...
                link: function ($scope, element, attrs, controller) {
                    googleApiPromise.then(function (google){
                      var keymap = {
                              tab: 9,
                              enter: 13,
                              esc: 27,
